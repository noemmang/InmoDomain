...
**Fase en curso:** Fase 7 — Analytics Service — diseño completo cerrado (índices, disparo del recálculo, columnas, arquitectura interna y fórmulas). Siguiente paso: empezar a escribir el proyecto `Analytics` (estructura de carpetas, entidades, migración de EF Core).
...
- [x] Diseñar tablas por base de datos lógica (una por servicio) — completado
  ...
  - **`analytics_db`** — completado, **ampliado en Fase 7**: `indice_precio`/`indice_actividad` originales pasan a representar explícitamente el **percentil histórico**; se añaden columnas equivalentes en **ranking nacional** para esos dos índices (nombres concretos pendientes de cerrar en Fase 7, ver más abajo). `indice_tendencia_precio` no tiene equivalente en ranking:
