...
**Fase en curso:** Fase 7 — Analytics Service — diseño completo cerrado. Escrito el esqueleto del proyecto `Analytics` (csproj, `Common/`, `Models/PropertyScore.cs`, `Data/AnalyticsDbContext.cs` + factory, `Program.cs`, `appsettings.json`), pendiente de `dotnet build` y de la migración `InitialCreate` por parte del usuario. Siguiente bloque de código: Repositories/Services/Controllers de los tres endpoints, y el consumidor de eventos.
...
- [ ] Bloques siguientes: proyecto `Analytics` (estructura de carpetas, `.csproj`), migración de EF Core de `analytics_db`, `AnalyticsCalculationService`, consumidor de eventos (`docker-compose` del emulador incluido), y los tres endpoints (ficha de provincia, comparador, `metric-value` interno).
- [x] Bloque 6 — Esqueleto del proyecto `Analytics` y entidad `PropertyScore`: escrito. Un solo proyecto (`backend/Analytics/Analytics.csproj`, mismas versiones de paquete que Property/Market Data: EF Core Design 10.0.12, Npgsql 10.0.3, Swashbuckle 10.2.3), con `Common/` (`Result`/`ResultError`, mismo patrón que Property), `Models/PropertyScore.cs` (las 6 columnas de índices + `Score` + `CalculatedAt`, sin FK en `codigo_provincia`), `Data/AnalyticsDbContext.cs` y `Data/AnalyticsDbContextFactory.cs` (mismo patrón pooled/direct que los demás servicios). Detectado y resuelto: la propiedad no puede llamarse `PropertyScore` dentro de la clase `PropertyScore` (error de compilación CS0542, un miembro no puede tener el mismo nombre que su tipo contenedor); renombrada a `Score`, mapeada igualmente a la columna `property_score`. Pendiente de verificar con `dotnet build` y de generar/aplicar la migración `InitialCreate` — la base de datos lógica `analytics_db` debe existir primero en el proyecto Neon `InmoDomain`, igual que se hizo con `market_data_db` en la Fase 5. Pendiente también fijar la precisión decimal de los seis índices y `property_score`: se ha usado `decimal(6,2)` de forma razonada pero no explícitamente confirmada por el usuario (rango -9999,99 a 9999,99, con margen sobre el 0-100 de los percentiles).
...
**Analytics**
- `GET /api/v1/provinces/{codigoProvincia}/analytics?from={periodo}&to={periodo}&trendQuarters={n}` — JWT opcional. `codigoProvincia` es el código INE. Sin token: histórico de `property_scores` + datos crudos de `compraventas_vivienda`/`valores_tasados` combinados desde Market Data (síncrono). Con token válido: añade campos personalizados (p. ej. `esFavorita`). `trendQuarters` (opcional): calcula en el momento de la petición la tendencia de precio frente a los N trimestres atrás pedidos, además de las dos variantes ya persistidas (`indice_tendencia_precio` frente al trimestre anterior, por defecto, e `indice_tendencia_precio_interanual` frente al mismo trimestre del año anterior); "sin datos suficientes" si no hay historia para el N pedido.
