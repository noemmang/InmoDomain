...
**Fase en curso:** Fase 4 — PostgreSQL + EF Core (Property Service), con la Fase 3 pendiente de cierre en el Bloque 5 de esta fase (`Program.cs`, `appsettings.json`, `Property.csproj` final)
**Última fase completada:** Fase 2 — Diseño de datos y contratos de API ✅
...
### Fase 3 — Primer microservicio (Property Service) 🔶 en curso
...
- [ ] `Program.cs`, `appsettings.json` y `Property.csproj` — pendientes, se completan cuando se retome esta fase.
- [ ] Implementación real de los Repositories con EF Core — se hace en la Fase 4.

### Fase 4 — PostgreSQL + EF Core 🔶 en curso
...
- [x] **Bloque 1 — Paquetes NuGet y conexión a Neon**
  ...
  - [x] Creado `Property.csproj` mínimo (SDK `Microsoft.NET.Sdk.Web`, `net10.0`, `Nullable`/`ImplicitUsings` habilitados) — se completará con su contenido final en el Bloque 5.
...
- [x] **Bloque 3 — Migraciones de EF Core**
  ...
  - [x] Creado un `Program.cs` mínimo y temporal (solo `WebApplication.CreateBuilder(args).Build().Run()`), imprescindible para que el proyecto compile con el SDK `Microsoft.NET.Sdk.Web` — sin él, `dotnet build` falla por falta de un punto de entrada. Se completa con su contenido real en el Bloque 5.
  ...
  - [ ] **Pendiente de higiene de seguridad**: la contraseña del rol `neondb_owner` quedó expuesta en el chat de trabajo durante la depuración de este bloque; pendiente regenerarla desde el panel de Neon y actualizar `ConnectionStrings:Property` y `ConnectionStrings:PropertyMigrations` con el valor nuevo.
- [ ] **Bloque 4** — Implementación real de `ProvinceRepository` y `FavoriteRepository`.
- [ ] **Bloque 5** — Cierre de la Fase 3: `Program.cs`, `appsettings.json` y `Property.csproj` final, con los Repositories ya registrados de verdad en DI.
