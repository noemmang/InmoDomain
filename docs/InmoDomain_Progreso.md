...
**Fase en curso:** Fase 5 — Market Data Service. Bloque 1 (integración real con INE y MIVAU) ✅ completado. Bloque 2 (arquitectura interna) parcialmente completado: la capa de lectura (`MarketData.Data` + `MarketData.Api`) ya está escrita, pero `HousingSale`/`MarketDataDbContext` necesitan corrección (quitar columna `regimen`) antes de generar la migración de EF Core de `market_data_db`. Bloque 3 (arquitectura de `MarketData.ImportJob`) ✅ diseño cerrado, pendiente de escribir el código.
...
- [x] **Bloque 2 — Arquitectura interna de Market Data (capa de lectura)** ✅ completado
  ...
  - [x] Escrito `MarketDataDbContext` con mapeo Fluent API completo de las 3 tablas (`compraventas_vivienda`, `valores_tasados`, `datos_cache`), incluidas las restricciones `CHECK` y los índices únicos ya definidos en la Fase 2. Entidades correspondientes en `MarketData.Data/Models/` (`HousingSale`, `AppraisedValue`, `CacheEntry`).
    - [ ] **Pendiente de corrección** (Fase 5/Bloque 3): la entidad `HousingSale` y el mapeo Fluent API de `compraventas_vivienda` en `MarketDataDbContext` todavía incluyen la columna `regimen` y su índice único de 4 columnas — hay que quitar `regimen`, dejando el índice único en `(codigo_provincia, periodo, estado_vivienda)`, antes de generar la migración `InitialCreate` (que sigue sin generarse, así que no hay conflicto de migración que deshacer).
...
- [x] **Bloque 3 — Arquitectura de `MarketData.ImportJob`** ✅ diseño cerrado, pendiente de implementar
